...
class Auth extends Model
{
    ...
    {
        $session = new Session();
        //Adicionei essa condição para colocar o cookie na sessão
        if ($cookie = filter_input(INPUT_COOKIE, "tempSES")) {
            $session->set("authUser", filter_var(base64_decode($cookie), FILTER_VALIDATE_INT));
        }
    ...
    {
        //elimina o cookie antes de matar a sessão
        setcookie("tempSES", null, time() - 28800, "/");
        $session = new Session();
    ...
    public function login(string $email, string $password, bool $save = false, int $level = 1): bool
    {
        ...
        $nickname = (new User())->find("email = :email", "email={$email}")->fetch();
        if ($nickname) {
            if ($save) {
        ...
        setcookie("tempSES", base64_encode($user->id), time() + 28800, "/");
        //LOGIN
