...
<div class="container-fluid">
    ...
    if (!$item) : ?>
        ...
        <form action="<?= url("/admin/user/invoices/edit/$invoice->id"); ?>" method="post">
            <!--ACTION SPOOFING-->
            <input type="hidden" name="action" value="update"/>
            <div class="form-row">
                <div class="form-group col-md-4">
                    <label for="first_name">Adicionar crédito</label>
                    <input type="text" name="invoice" value="<?= $invoice->invoice;?>" placeholder="Adicionar crédito" class="form-control mask-money"
                           required>
                </div>
                ...
                <div class="form-group col-md-4">
                    ...
                    <select name="status" id="" class="form-control">
                        <?php foreach (invoicesStatus() as $key => $p): ?>
                            <option value="<?= $key; ?>" <?= $invoice->status == $key ? ' selected' : '';?>><?= $p; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
        ...
        <form action="<?= url("/admin/options/option/{$item->id}"); ?>" method="post">
            ...
            <div class="form-row">
                <div class="form-group col-md-4">
                    ...
                    <input type="text" name="item" value="<?= $item->item; ?>" placeholder="Item opcional"
